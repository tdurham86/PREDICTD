...
def _compile_bdg_and_upload(ctassay_part, out_bucket, out_root, bdg_path, make_public=True, tmpdir='/data/tmp', sort_bdg=False, no_bw=False):
    ...
    for ct_name, assay_name in ctassay_part:
        for bdg_type in ['imp', 'obs']:
            ...
            if sort_bdg is True:
                local.env['LC_ALL'] = 'C'
                unixcat, unixpaste, unixsort, bedtools = local['cat'], local['paste'], local['sort'], local['bedtools']
                chain = unixcat[bdg_paths] | unixpaste[os.path.join(tmpdir, 'bdg_coords.txt'), '-'] | unixsort['-k1,1', '-k2,2n', '-T', tmpdir, '-S', '8g'] | bedtools['intersect', '-wa', '-a', 'stdin', '-b', chrom_bed_path, '-f', '1.0'] > out_bdg
            else:
...
def _combine_bdg_coords(bdg_coord_glob):
    ...
    if os.path.isfile(bdg_coord_path):
        raise Exception('bdg_coord_path already exists: {!s}'.format(bdg_coord_path))
    unixcat = local['cat']
